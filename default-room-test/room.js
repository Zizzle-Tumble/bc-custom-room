...
whitelisted = JSON.parse(localStorage.getItem("whitelist"))
...
joinconf = confirm("Do you want to enter personal room "+newroom.name+"? [Note:Players Can still see what you say!]")
   alert("NOTE: YOU CAN WHITELIST WHO YOU CAN SEE BY TYPING whitelist(``id of player``) IN THE CONSOLE!")
if (joinconf == true){
joinnewroom() 
playrhide()
   } else {
...
reload = function(){
 ...
 for (RoomLoop = 0; RoomLoop < world.room.playerCrumbs.length; RoomLoop++){
     if (world.stage.room.players[world.room.playerCrumbs[RoomLoop].i].visible == true && world.stage.room.players[world.room.playerCrumbs[RoomLoop].i].playerId != world.player.playerId && whitelisted.includes([world.room.playerCrumbs[RoomLoop].i].playerId) != true && world.room.roomId == newroom.roomId){
...
function whitelist(playerid){
   whitelisted.push(playerid)
   localStorage.setItem("whitelist",JSON.stringify(whitelisted))
}
